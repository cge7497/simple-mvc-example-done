...
For our name, we define a type of String. This means name is a string. Since mongo stores javascript
 objects, the type usually does not matter. However, this is something added by Mongoose to clarify
 what each variable should contain. We also give it the required option, which means every object in
 the cat database MUST have a name. We set the trim option to true, which will remove trailing and
 leading whitespace from the string. Finally we set the unique option to true. This means that there
 cannot be two cats with the same name.

 For our bedsOwned, we say it is a number. We set the minimum value to 0. We say it is required.

 For our createdDate, we want to store the time that the cat was first added to the database. Rather
 than figure that out in our own code when we create each cat, we can instead set it's default to
 the Date.now function. When a new cat is made, it will execute the default function and return
 the current date.
*/
